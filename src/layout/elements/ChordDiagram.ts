...
import types, { Alignment, DEFAULT_SERIF_FONT_FAMILY, LINE_STROKE_WIDTH, STAFF_LINE_HEIGHT } from "..";
...
export class ChordDiagram extends LayoutElement<"ChordDiagram", types.LineElement> implements types.ChordDiagram {
  ...
  render(context: CanvasRenderingContext2D): void {
    const textBox = new Box(this.box.x, this.box.y, this.box.width, STAFF_LINE_HEIGHT);
    ...
    const diagramBox = new Box(
      ...
      this.box.width,
      this.box.height - textBox.height
    );
    context.translate(0, textBox.height);
    FretboardDiagram(context, this, diagramBox);
  }
}

// TODO have ChordDiagram just be FretboardDiagram, and have layout use a group to show other text elements

const FretboardDiagram = (context: CanvasRenderingContext2D, chordDiagram: ChordDiagram, box: Box) => {
  const part = ancestorOfType<ChordDiagram, Part>(chordDiagram, "Part");
  const numStrings = part?.part?.instrument?.tuning?.length;
  if (!numStrings) {
    return null;
  }
