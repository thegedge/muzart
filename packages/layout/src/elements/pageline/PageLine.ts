...
export class PageLine extends SimpleGroup<"PageLine", AnyLayoutElement> {
  readonly type = "PageLine";

  private aboveStaffLayout: AboveStaff;
  private staffLayout: FlexGroupElement<AnyLayoutElement>;
  ...
  addElement(element: AnyLayoutElement, factor?: number) {
  ...
  tryAddElement(element: AnyLayoutElement, factor?: number) {
    const wasAdded = this.staffLayout.tryAddElement(element, factor);
    if (wasAdded && element instanceof Measure) {
      this.measures.push(element);
    }
    this.dirty ||= wasAdded;
    return wasAdded;
